...
- (2026-08-25) Follow-up: серверный путь всё ещё теряет sentinel «как в системе» — `_persist()` в `lib/application/settings/app_settings_cubit.dart` (~126) разворачивает null-локаль в конкретный язык через `_resolveAppLocale` (~193), и это значение потом возвращается с сервера как «выбор пользователя». Поведение существовало до ветки `feat/pre-auth-appearance`, там сознательно не тронуто; сделать sentinel-preserving отдельной задачей.
